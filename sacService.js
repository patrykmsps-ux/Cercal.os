...
// ── Cookie ────────────────────────────────────────────────────
// Prioridade: 1) env FUSION_COOKIE  2) arquivo fusion_cookie.txt  3) fixo
function lerCookie() {
  if (process.env.FUSION_COOKIE && process.env.FUSION_COOKIE.trim()) {
    return process.env.FUSION_COOKIE.trim();
  }
  try {
    if (fs.existsSync(COOKIE_FILE)) {
      const c = fs.readFileSync(COOKIE_FILE, "utf8").trim();
      if (c && !c.includes("placeholder")) return c;
    }
  } catch (_) {}
  return COOKIE_FIXO;
}
